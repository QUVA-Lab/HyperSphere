...
if __name__ == '__main__':
	...
	ndata = 20
	ndim = 1
	model_for_generating = GPRegression(kernel=Matern52(ndim))
	train_x = torch.FloatTensor(ndata, ndim).uniform_(-2, 2)
	chol_L = torch.potrf((model_for_generating.kernel(train_x) + torch.diag(model_for_generating.likelihood(train_x))), upper=False)
	...
	model_for_learning = GPRegression(kernel=Matern52(ndim))
	model_for_generating.init_parameters()
